...
export default function ConfirmationPage() {
  ...
  if (!booking) return <div style={{ padding: 50, textAlign: 'center' }}>{t('common.error')}</div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div className="cp-card animate-fadeIn" style={{ maxWidth: 500, width: '100%', padding: 40, textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: 32, fontWeight: 900, marginBottom: 8 }}>{t('booking.bookingConfirmed')}</h1>
        <p style={{ color: 'var(--muted)', marginBottom: 32 }}>{t('booking.confirmedMessage')}</p>

        <div style={{ background: 'var(--bg)', borderRadius: 16, padding: 24, textAlign: 'left', marginBottom: 32, border: '1px dashed var(--border)' }}>
          <div style={{ fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', fontWeight: 600, marginBottom: 4 }}>{t('booking.bookingRef')}</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--accent)', marginBottom: 20 }}>{booking.bookingRef}</div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            <div>
              <div style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase' }}>{t('movie.title')}</div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>{booking.show.movie.title}</div>
            </div>
            <div>
              <div style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase' }}>{t('ticket.dateTime')}</div>
